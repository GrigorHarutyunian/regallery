import webSiteSection from "../../assets/imgs/websitesection3.png";
const aboutMobileResponsivenessData = {
  title: "Mobile Optimized",
  text: "It is not a surprise for modern websites, but we are proud that our plugin is fully compatible with mobile and 4k screens.",
  img: webSiteSection,
  sizes:
    "(max-width: 700px) 298px, (min-width: 701px) and (max-width: 1100px) 397px, 595px",
  alt: `Discover why ReGallery is the best WordPress photo gallery plugin. Featuring responsive design, intuitive interface, and extensive customization options.`,
};
export default aboutMobileResponsivenessData;
